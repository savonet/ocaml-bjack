...
/* most other ocaml-bjack functions it provides a good point to attempt reconnection */
/* */
...
#define getDriver(x) _getDriver(x,fprintf(OUTFILE, "%s::%s(%d) getting driver %s\n", __FILE__, __FUNCTION__, __LINE__,x->client_name)); TRACE("got driver %s\n",x->client_name);
jack_driver_t *
_getDriver(jack_driver_t *drv, int ignored)
{
  fflush(OUTFILE);
#else
jack_driver_t *
...
*/
#if TRACE_getReleaseDevice
#define releaseDriver(x) TRACE("releasing driver %s\n",x->client_name); _releaseDriver(x);
void
...
JACK_OpenDevice(jack_driver_t * drv)
{
  ...
  TRACE("client name '%s'\n", drv->client_name);
  TRACE("server name '%s'\n", drv->server_name);
  if (strcmp(drv->server_name,"")!=0) options = JackServerName|options;
  if((drv->client = jack_client_open(drv->client_name, options, NULL, drv->server_name)) == 0)
  {
    /* try once more */
 ...
 */
int
JACK_Open(jack_driver_t *drv, unsigned int bits_per_channel,
...
JACK_Close(jack_driver_t *drv)
{
  ...
  if (drv->client_name) free(drv->client_name);
  drv->client_name = NULL;

  if (drv->server_name) free(drv->server_name);
...
long
JACK_Write(jack_driver_t *drv, unsigned char *data, unsigned long bytes)
{
  getDriver(drv);
  if (drv->in_use != TRUE) 
  {
...
long
JACK_Read(jack_driver_t *drv, unsigned char *data, unsigned long bytes)
{
  getDriver(drv);
  if (drv->in_use != TRUE) 
  {
...
int
JACK_SetVolumeForChannel(jack_driver_t *drv, unsigned int channel,
                         unsigned int volume)
{
  getDriver(drv);
...
int
JACK_SetAllVolume(jack_driver_t *drv, unsigned int volume)
{
  unsigned int i;
...
void
JACK_GetVolumeForChannel(jack_driver_t *drv, unsigned int channel,
                         unsigned int *volume)
{
...
JACK_SetVolumeEffectType(jack_driver_t *drv, enum JACK_VOLUME_TYPE type)
{
...
int
JACK_SetState(jack_driver_t *drv, enum status_enum state)
{
  getDriver(drv);
...
enum status_enum
JACK_GetState(jack_driver_t *drv)
{
...
unsigned long
JACK_GetOutputBytesPerSecond(jack_driver_t *drv)
{
...
unsigned long
JACK_GetInputBytesPerSecond(jack_driver_t *drv)
{
...
unsigned long
JACK_GetBytesStored(jack_driver_t *drv)
{
  getDriver(drv);
...
unsigned long
JACK_GetBytesFreeSpace(jack_driver_t *drv)
{
  getDriver(drv);
...
unsigned long
JACK_GetBytesUsedSpace(jack_driver_t *drv)
{
  long return_val;
...
long
JACK_GetPosition(jack_driver_t * drv, enum pos_enum position,
                           int type)
{
  long return_val = 0;
  struct timeval now;
...
//   internal variables
void
JACK_SetPosition(jack_driver_t *drv, enum pos_enum position, long value)
{
  getDriver(drv);
  double sec2msFactor = 1000;
#if TRACE_ENABLE
  ...
  TRACE("client %s input_value of %ld %s, new value of %ld, setting position_byte_offset to %ld\n",
        drv->client_name, input_value, (position == MILLISECONDS) ? "ms" : "bytes",
...
unsigned long
JACK_GetBytesPerOutputFrame(jack_driver_t *drv)
{
...
unsigned long
JACK_GetBytesPerInputFrame(jack_driver_t *drv)
{
...
long
JACK_GetMaxOutputBufferedBytes(jack_driver_t *drv)
{
  long return_val;
...
long
JACK_GetMaxInputBufferedBytes(jack_driver_t *drv)
{
  long return_val;
...
int
JACK_GetNumOutputChannels(jack_driver_t *drv)
{
...
int
JACK_GetNumInputChannels(jack_driver_t *drv)
{
...
long
JACK_GetSampleRate(jack_driver_t *drv)
{
...
unsigned long
JACK_GetJackBufferedBytes(jack_driver_t *drv)
{
  long return_val;
...
/* consider mapping them in the ocaml-bjack.h header file since its useless to the user unless */
