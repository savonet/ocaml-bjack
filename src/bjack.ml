...
let _ = 
  ...
  Callback.register_exception "bio2jack_exn_bytes_per_output_frame_invalid" Bytes_per_output_frame_invalid ;
  Callback.register_exception "bio2jack_exn_bytes_per_input_frame_invalid" Bytes_per_input_frame_invalid ;
  Callback.register_exception "bio2jack_exn_too_many_output_channels" Too_many_output_channels ;
  Callback.register_exception "bio2jack_exn_port_name_output_channel_mismatch" Port_name_output_channel_mismatch ;
  Callback.register_exception "bio2jack_exn_port_not_found" Port_not_found ;
  Callback.register_exception "bio2jack_exn_too_many_input_channels" Too_many_input_channels ;
  Callback.register_exception "bio2jack_exn_port_name_input_channel_mismatch" Port_name_input_channel_mismatch
...
external set_conversion_function : int -> unit = "caml_bjack_set_conversion_function"

let set_conversion_function c = 
  set_conversion_function (int_of_converter c)
...
let iport_flags flags = 
  List.fold_left (fun x y -> x lor (int_of_port_flag y)) 
      0 flags

external open_t : int -> int -> string -> string -> int -> int -> int -> int -> t = "caml_bjack_open_byte" "caml_bjack_open"

let open_t ?(ringbuffer_size=4096) ?(server_name="")
              ~rate ~bits_per_sample 
             ~input_channels ~output_channels 
             ~flags ~client_name () = 
  let flags = iport_flags flags in
  open_t bits_per_sample rate client_name server_name
        input_channels output_channels 
        flags ringbuffer_size
...
let get_position ~position_unit ~position_type device = 
  get_position device (int_of_position_unit position_unit)
...
let set_state device state = 
  set_state device (int_of_playing_state state)

external get_state : t -> int = "caml_bjack_set_state"

let get_state device = 
...
external get_jack_buffered_bytes : t -> int = "caml_bjack_get_jack_buffered_bytes"
...
external set_volume_effect_type : t -> int -> int = "caml_bjack_set_volume_effect_type"
...
external set_channel_volume : t -> int -> int -> unit = "caml_bjack_set_channel_volume"
...
external get_output_bytes_per_second : t -> int = "caml_bjack_get_output_bytes_per_second"

external get_input_bytes_per_second : t -> int = "caml_bjack_get_input_bytes_per_second"
...
external get_bytes_per_output_frame : t -> int = "caml_bjack_get_bytes_per_output_frame"

external get_bytes_per_input_frame : t -> int = "caml_bjack_get_bytes_per_input_frame"

external get_num_output_channels : t -> int = "caml_bjack_get_num_output_channels"
